...
public class AgentFactory {
    ...
    public Agent createReActAgent(AgentConfig config) {
        PromptTemplate promptTemplate = promptRegistry.getTemplate(
                properties.getReact().getPromptTemplateId());
    ...
    public Agent createAgent(AgentType type, AgentConfig config) {
        switch (type) {
            ...
            case PLAN_AND_EXECUTE:
                return createPlanAndExecuteAgent(config);
            case CONVERSATIONAL:
                // TODO 实现对话式代理
                throw new UnsupportedOperationException("对话式代理尚未实现");
            default:
