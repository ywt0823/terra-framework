package com.terra.framework.nova.model.deepseek;
 ...
 */
@Slf4j
public class DeepSeekModel extends AbstractAIModel {
    ...
    public DeepSeekModel(ModelConfig config, HttpClientUtils httpClient) {
        ...
        // 创建参数映射策略
        DeepSeekRequestMappingStrategy mappingStrategy = new DeepSeekRequestMappingStrategy();

        // 创建模型适配器
        this.adapter = new DeepSeekAdapter(mappingStrategy, authProvider);
    }
    ...
    public ModelResponse generate(String prompt, Map<String, Object> parameters) {
        try {
            ...
            // 转换为DeepSeek请求
            JSONObject deepseekRequest = adapter.convertRequest(request, JSONObject.class);
    ...
    public void init() {
        try {
            log.info("初始化DeepSeek模型: {}", config.getModelId());
            ...
            log.error("初始化DeepSeek模型失败", e);
            status = ModelStatus.ERROR;
            throw new ModelException("初始化DeepSeek模型失败: " + e.getMessage(), e);
        }
    }

    @Override
    public void close() {
        log.info("关闭DeepSeek模型: {}", config.getModelId());
    ...
    private String getModelName() {
        ...
        // 如果模型ID包含冒号（如deepseek:deepseek-chat），则提取真实模型名
        if (modelId.contains(":")) {
