package com.terra.framework.nova.core.model.openai;
 ...
 */
@Slf4j
public class OpenAIModel extends AbstractAIModel {
    ...
    public OpenAIModel(ModelConfig config, HttpClientUtils httpClient) {
        ...
        // 创建参数映射策略
        OpenAIRequestMappingStrategy mappingStrategy = new OpenAIRequestMappingStrategy();

        // 创建模型适配器
        this.adapter = new OpenAIAdapter(mappingStrategy, authProvider);
    }
    ...
    public ModelResponse generate(String prompt, Map<String, Object> parameters) {
        try {
            ...
            // 转换为OpenAI请求
            JSONObject openaiRequest = adapter.convertRequest(request, JSONObject.class);
    ...
    public void init() {
        try {
            log.info("初始化OpenAI模型: {}", config.getModelId());
            ...
            // 获取模型详情以获取更准确的信息（可选，未来可实现）
            // retrieveModelInfo(modelName);

            status = ModelStatus.READY;
        } catch (Exception e) {
            log.error("初始化OpenAI模型失败", e);
            status = ModelStatus.ERROR;
            throw new ModelException("初始化OpenAI模型失败: " + e.getMessage(), e);
        }
    }

    @Override
    public void close() {
        log.info("关闭OpenAI模型: {}", config.getModelId());
    ...
    private String getModelName() {
        ...
        // 如果模型ID包含冒号（如openai:gpt-4），则提取真实模型名
        if (modelId.contains(":")) {
